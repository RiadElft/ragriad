...
def getgroops(request):
    try:
        print("Attempting to fetch groops...")
        groops = Groop.objects.all()

        if not groops:
            return JsonResponse({"message": "No groops found"})

        groops_data = []
        for groop in groops:
            groop_data = {
                "id": str(groop._id),
                "name": groop.groopName,
            }
            groops_data.append(groop_data)

        return JsonResponse({"groops": groops_data}, safe=False)
